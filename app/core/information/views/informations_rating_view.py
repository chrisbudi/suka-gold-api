...
from core.information.serializers import InformationRatingSerializer as infoSerializer
...
def fetch_rating(request):
    try:
        ...
        return Response("Information Rating does not exist", status=status.HTTP_404_NOT_FOUND)
...
@api_view(['PATCH'])
def patch_rating(request, id):
    try:
        info = modelInfo.objects.get(pk=id)
        
    except info.DoesNotExist:
        return Response({'error': 'Information Rating not found'}, status=status.HTTP_404_NOT_FOUND)
...
def delete_rating(request, id):
    try:
        ...
        return Response({
            "error": "Information Rating not found"
