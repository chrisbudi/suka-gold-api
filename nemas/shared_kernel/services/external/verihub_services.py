...
class VerihubService:
    ...
    def liveness_photo_file(self, payload=None):
        """
        Verify Liveness photo file using Verihub API.
        ...
        try:
            response = requests.post(
                self.base_url + settings.VERIHUB["LIVENESS_PHOTO"],
                headers=self.headers,
                json=payload,
            )
            return response.json()
        except Exception as e:
            raise Exception(f"Failed to verify Liveness photo file: {str(e)}")
